...
<body>
	...
	<div align="center" class="pagingArea">
		<% if(currentPage != 1){ %>
			<button onclick="location.href='<%= contextPath %>/lowerListArea.ht?currentPage=<%= currentPage-1 %>';"><</button>
		<% } %>
		...
		<% for(int p=startPage; p<=endPage; p++){ %>
			...
			<% }else{ %>
				<button onclick="location.href='<%= contextPath %>/lowerListArea.ht?currentPage=<%= p %>';"><%= p %></button>
			<% } %>
		<% } %>
