						...
						<div class="outer2">
							...
							<!-- 1을 누르면 "<"이 안보이고 마지막 숫자버튼을 누르면 ">"이 안보이도록 조건 처리해야 함-->
								<div align="center" class="pagingArea">
									<% if(currentPage != 1){ %>
									<button type="button" onclick="location.href='<%= contextPath %>/htBookmark.md?currentPage=<%= currentPage-1 %>';"><</button>
									<% } %>
									...
									<% for(int p=startPage; p<=endPage; p++){ %>
										...
										<% }else{ %>
											<button onclick="location.href='<%= contextPath %>/htBookmark.md?currentPage=<%= p %>';"><%= p %></button>
										<% } %>
									<% } %>
																
									<% if(currentPage != maxPage){ %>
										<button onclick="location.href='<%= contextPath %>/htBookmark.md?currentPage=<%= currentPage + 1 %>';">></button>
									<% } %>
								</div>
						   <div class="Btns">
							   <button type="button" id="submitBtn" onclick="location.href='<%= contextPath %>/updateHtBookmark.md?currentPage=1'">수정</button>
						   </div>
						</div>
					</div>
