...
/**
 * Servlet implementation class CommFreeListServlet
 */
@WebServlet("/free.co")
public class CommFreeListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CommFreeListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new CommService().freeSelectListCount(); 
		...
		ArrayList<Comm> list = new CommService().freeSelectList(pi);
		
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		
		request.getRequestDispatcher("views/member/comm/commFreeListView.jsp").forward(request, response);
