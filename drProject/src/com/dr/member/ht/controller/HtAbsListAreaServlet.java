...
/**
 * Servlet implementation class HtCoreListViewServlet
 */
@WebServlet("/absListArea.ht")
public class HtAbsListAreaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HtAbsListAreaServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 페이징 처리
				...
				String category = "복부운동"; // 해당 리스트 카테고리 조회
				
				// value : 조회 방식 선택 값 
				String value = request.getParameter("value");
				
				// listCount : 총 게시글 갯수 조회해서 담기
				listCount = new HtService().selectAbsListCount(category);
				...
				ArrayList<Ht> list = new HtService().selectAbsHtList(category, value ,pi);
				
				
				request.setAttribute("pi", pi);
				request.setAttribute("list", list);
				request.getRequestDispatcher("views/member/ht/htUpperListArea.jsp").forward(request, response);
	}
