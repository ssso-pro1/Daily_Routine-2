...
/**
 * Servlet implementation class HtBodyListViewServlet
 */
@WebServlet("/bodyListArea.ht")
public class HtBodyListAreaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HtBodyListAreaServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String category = "전신운동"; // 해당 리스트 카테고리 조회
		
		// value : 조회 방식 선택 값 
		String value = request.getParameter("value");
		
		// listCount : 총 게시글 갯수 조회해서 담기
		listCount = new HtService().selectBodyListCount(category);
		...
		ArrayList<Ht> list = new HtService().selectBodyHtList(category, value, pi);
		
		
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		request.getRequestDispatcher("views/member/ht/htBodyListArea.jsp").forward(request, response);
	}
