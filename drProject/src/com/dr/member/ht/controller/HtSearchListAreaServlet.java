 ...
 */
@WebServlet("/searchListArea.ht")
public class HtSearchListAreaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HtSearchListAreaServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new HtService().selectSearchListCount(searchTitle);
		...
		ArrayList<Ht> list = new HtService().selectSearchHtList(value, searchTitle, pi);
		// 리스트 담김
		
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		request.getRequestDispatcher("views/member/ht/htSearchListArea.jsp").forward(request, response);
