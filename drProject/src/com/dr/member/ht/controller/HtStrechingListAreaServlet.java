...
/**
 * Servlet implementation class HtStrechingListServlet
 */
@WebServlet("/strechingListArea.ht")
public class HtStrechingListAreaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HtStrechingListAreaServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 페이징 처리
				...
				String category = "스트레칭"; // 해당 리스트 카테고리 조회
				
				// value : 조회 방식 선택 값 
				String value = request.getParameter("value");
				
				// listCount : 총 게시글 갯수 조회해서 담기
				listCount = new HtService().selectStrechingListCount(category);
				...
				ArrayList<Ht> list = new HtService().selectStrechingHtList(category, value, pi);
				
				
				request.setAttribute("pi", pi);
				request.setAttribute("list", list);
		request.getRequestDispatcher("views/member/ht/htStrechingListArea.jsp").forward(request, response);
	}
